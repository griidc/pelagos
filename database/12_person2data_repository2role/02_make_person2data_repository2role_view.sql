...
$$
BEGIN
IF EXISTS (SELECT relname FROM pg_class WHERE relname = 'person2data_repository2role')
THEN
    DROP TRIGGER IF EXISTS udf_person2data_repository2role_delete_trigger ON person2data_repository2role;
    DROP TRIGGER IF EXISTS udf_person2data_repository2role_insert_trigger ON person2data_repository2role;
    DROP TRIGGER IF EXISTS udf_person2data_repository2role_update_trigger ON person2data_repository2role;
ELSE
    RAISE NOTICE 'person2data_repository2role view does not exist, so no triggers to drop. Skipping.';
END IF;
END
$$;

DROP FUNCTION IF EXISTS udf_modify_person2data_repository2role();

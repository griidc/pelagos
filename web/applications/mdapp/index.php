...
$app->post(
    ...
    function () use ($app) {
        ...
        try {
            if (!isset($_FILES['newMetadataFile']['error']) || is_array($_FILES['newMetadataFile']['error'])) {
            ...
            */
            ...
            if ($_FILES['newMetadataFile']['size'] == 0) {
                throw new RuntimeException(
                    'File uploaded was empty.  This may be caused by the use of the browser back button.'
                );
            }
            ...
            if (!preg_match('/-metadata.xml$/', $orig_filename)) {
                throw new RuntimeException(
                    "Bad filename $orig_filename. Filename must be in the form of UDI-metadata.xml."
                );
            }
            ...
            if (!$tmpp) {
                ...
                $err_str = $err->message;
                throw new RuntimeException(
                    "Malformed XML: The XML file supplied $orig_filename could not be parsed. ($err_str)"
                );
            }
            ...
            if (isset($_POST['validateSchema']) and $_POST['validateSchema'] == 'on') {
                    ...
                    for ($i=0; $i<sizeof($errors); $i++) {
                        switch ($errors[$i]->level) {
                            case LIBXML_ERR_WARNING:
                                $error = "WARNING (".$errors[$i]->code.") on XML line ";
                                $error .= $errors[$i]->line.": ".$errors[$i]->message;
                                drupal_set_message($error, 'warning');
                                break;
                            case LIBXML_ERR_ERROR:
                                $schemaErrors++;
                                $error = "ERROR (".$errors[$i]->code.") on XML line ";
                                $error .= $errors[$i]->line.": ".$errors[$i]->message;
                                drupal_set_message($error, 'error');
                                break;
                            case LIBXML_ERR_FATAL:
                                $schemaErrors++;
                                $error = "FATAL ERROR (".$errors[$i]->code.") on XML line ";
                                $error .= $errors[$i]->line.": ".$errors[$i]->message;
                                drupal_set_message($error, 'error');
                                break;
                        }
                ...
                if ($schemaErrors > 0) {
                    throw new RuntimeException(
                        "The file uploaded as $orig_filename does not validate as ISO-19115-2 compliant XML."
                    );
                }
            }
            ...
            if (isset($loc_1[0][0])) {
                $loc_1_val = $loc_1[0][0];
                $errmsg = textboxize(
                    "GRIIDC standards: Filename, $orig_filename does not match file identifier, $loc_1_val as UDI-metadata.xml (w/dash for colon)",
                    "/gmi:MI_Metadata/gmd:fileIdentifier[1]/gco:CharacterString[1]"
                );
                ...
                $errmsg = textboxize(
                    "GRIIDC standards:  File identifier not set in file uploaded.",
                    "/gmi:MI_Metadata/gmd:fileIdentifier[1]/gco:CharacterString[1]"
                );
                if (isset($_POST['test1']) and $_POST['test1']=='on') {
                    array_push($errors, $errmsg);
            ...
            if (isset($loc_2[0][0])) {
                $loc_2_val = $loc_2[0][0];
                $errmsg = textboxize(
                    "GRIIDC standards:  UDI, $udi does not match metadata URL, $loc_2_val",
                    "/gmi:MI_Metadata/gmd:dataSetURI/gco:CharacterString"
                );
                ...
                $errmsg = textboxize(
                    "GRIIDC standards:  Metadata URL not set in the file uploaded.",
                    "/gmi:MI_Metadata/gmd:dataSetURI/gco:CharacterString"
                );
                if (isset($_POST['test2']) and $_POST['test2']=='on') {
                    array_push($errors, $errmsg);
            ...
            if (isset($loc_3[0][0])) {
                $loc_3_val = $loc_3[0][0];
                $errmsg = textboxize(
                    "GRIIDC standards:  UDI, $udi does not match distribution URL $loc_3_val.",
                    "/gmi:MI_Metadata/gmd:distributionInfo[1]/gmd:MD_Distribution[1]/gmd:distributor[1]/gmd:MD_Distributor[1]/gmd:distributorTransferOptions[1]/gmd:MD_DigitalTransferOptions[1]/gmd:onLine[1]/gmd:CI_OnlineResource[1]/gmd:linkage[1]/gmd:URL[1]"
                );
                ...
                $errmsg = textboxize(
                    "GRIIDC standards:  Distribution URL was not set in file uploaded.",
                    "/gmi:MI_Metadata/gmd:distributionInfo[1]/gmd:MD_Distribution[1]/gmd:distributor[1]/gmd:MD_Distributor[1]/gmd:distributorTransferOptions[1]/gmd:MD_DigitalTransferOptions[1]/gmd:onLine[1]/gmd:CI_OnlineResource[1]/gmd:linkage[1]/gmd:URL[1]"
                );
                if (isset($_POST['test3']) and $_POST['test3']=='on') {
                    array_push($errors, $errmsg);
            ...
            try {
                ...
                $geo = $xml->xpath(
                    '/gmi:MI_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/gmd:extent/gmd:EX_Extent/gmd:geographicElement/gmd:EX_BoundingPolygon/gmd:polygon/*'
                );
                if ($geo) {
                    // double check for existance of geometry ( in case user override )
...
EOF;
        ...
        if (isset($envelope_wkt) and ($envelope_wkt != null)) {
            ...
            drupal_add_js(
                "/$GLOBALS[PAGE_NAME]/js/mdapp-ul.js",
                array('type'=>'external')
            );
        }
...
function index($app)
{
    ...
    if (isset($_SESSION['testPolygon'])) {
        $stash['testPolygon'] = $_SESSION['testPolygon'];
    }
    return $stash;
}
 ...
 */
$app->put(
    '/jiraLink/:udi/(:linkValue)/',
    function ($udi, $jiraLinkValue = null) use ($app) {
        global $user;
...
function sendEmail($to, $from, $sub, $message, $cc = null)
{
    ...
    if ($GLOBALS['module_config']['email']['disable_email'] == 1) {
        ...
        drupal_set_message(
            "The following email was disabled by ini setting. The following message was not sent.<br />To: $to<br />Cc: $cc_str<br />From: $from<br />Subject: $sub<br />SMTP Server: $smtp_server<br />Message: $message",
            'warning'
        );
    } else {

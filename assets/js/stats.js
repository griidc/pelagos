...
const datasetsIcon = require('../images/icon-datasets.png');
const filesizeIcon = require('../images/icon-filesize.png');
const downloadsIcon = require('../images/icon-downloads.png');
const projectsIcon = require('../images/icon-projects.png');
const researchersIcon = require('../images/icon-researchers.png');
...
function loadRepositorySummary() {
  ...
  axios.get(url)
    .then((response) => {
      ...
      document.getElementById('datasets-count').textContent = totalDatasets || '-';

      // Update total size
      const datasetSize = totalSize?.split(' ');
      if (datasetSize && datasetSize.length > 0) {
        const [sizeValue, sizeUnit] = datasetSize;
        document.getElementById('totalsize-count').textContent = sizeValue;
        document.getElementById('size-unit').textContent = sizeUnit;
      }
      ...
      document.getElementById('researchgroups-count').textContent = researchGroupCount || '-';

      // Update downloads count
      document.getElementById('downloads-count').textContent = totalDownloadCount || '-';
    })
    .catch(() => {
      // Error loading repository summary - data will remain as '-'
    });
}
...
function loadDatasetOverTimeChart() {
  ...
  axios.get(url)
    ...
    .catch(() => {
      // Error loading dataset over time chart
    });
}
...
function loadDatasetSizeRangesChart() {
  ...
  axios.get(url)
    ...
    .catch(() => {
      // Error loading dataset size ranges chart
    });
}

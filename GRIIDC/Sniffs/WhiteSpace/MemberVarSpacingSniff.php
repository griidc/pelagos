<?php
/**
 * Verifies that class member variables are spaced correctly.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * Verifies that class member variables are spaced correctly.
 *
 * Based on Squiz_Sniffs_WhiteSpace_MemberVarSpacing.
 * (Modified to not require a blank line before the first member
 * variable if it is the first thing defined in the class.)
 *
 * @category  PHP
 * @package   PHP_CodeSniffer
 */
class GRIIDC_Sniffs_WhiteSpace_MemberVarSpacingSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    ...
    protected function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $prevCurly = $phpcsFile->findPrevious(T_OPEN_CURLY_BRACKET, ($first - 1), null);
        if ($tokens[$prevCurly]['line'] === ($tokens[$first]['line'] - 1)) {
            // If there's an open curly brace on the line before,
            // it's the first member var. Don't require space.
            return;
        }

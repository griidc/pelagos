 ...
 *
 * Based on Squiz_Sniffs_WhiteSpace_FunctionSpacingSniff.
 * (Modified to not require blank lines before the first function
 * in a class if it is the first thing defined or after the last
 * function in a class if it is the last thing defined).
 *
 * @category  PHP
 * @package   PHP_CodeSniffer
 */
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        */
        ...
        if ($nextLineToken === ($phpcsFile->numTokens - 1)) {
            ...
            if ($nextContent === false) {
                ...
                $nextCurly = $phpcsFile->findNext(T_CLOSE_CURLY_BRACKET, $nextLineToken, null);
                if ($tokens[$nextCurly]['line'] === $tokens[$nextLineToken]['line']) {
                    // Don't require space after last function in a class.
                    $foundLines = $this->spacing;
                } else {
        ...
        */
        ...
        if (is_null($prevLineToken) === true) {
            ...
            $prevCurly = $phpcsFile->findPrevious(T_OPEN_CURLY_BRACKET, $prevContent, null);
            if ($tokens[$prevCurly]['line'] === $tokens[$prevContent]['line']) {
                // If there's an open curly brace on the line before,
                // it's the first function. Don't require space.
                return;
            }
